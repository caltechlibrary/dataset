...
darwin-amd64: 
	env PATH="$(DARWIN_TOOLS):$(PATH)" CGO_ENABLED=1 GOOS=darwin 
GOARCH=amd64 CC=o64-clang go build -buildmode=c-shared -o 
"$(LIB_NAME).dylib" "$(LIB_NAME).go"

windows-amd64: 
	env PATH="$(WINDOWS_TOOLS):$(PATH)" CGO_ENABLED=1 GOOS=windows 
GOARCH=amd64 CC=w64-clang go build -buildmode=c-shared -o 
"$(LIB_NAME).dll" "$(LIB_NAME).go"
...
save:
	if [ "$(msg)" != "" ]; then git commit -am "$(msg)"; else git 
commit -am "Quick Save"; fi
...
release-darwin-amd64: darwin-amd64 FORCE
	...
	cd ../dist && tar zcvf $(LIB_NAME)-$(VERSION)-macosx-amd64.tar.gz 
$(LIB_NAME).dylib $(LIB_NAME).h  README.md LICENSE INSTALL.md
...
release-windows-amd64: windows-amd64 FORCE
	...
	cp -v $(LIB_NAME).h ../dist/
	cd ../dist && tar zcvf $(LIB_NAME)-$(VERSION)-windows-amd64.tar.gz 
$(LIB_NAME).dll $(LIB_NAME).h  README.md LICENSE INSTALL.md
