%dataset3d(1) user manual | version 3.0.0-alpha 311aa47
% R. S. Doiel
% 2025-05-28
...
dataset3d [OPTIONS] SETTINGS_FILE

# DESCRIPTION

dataset3d provides a web service for one or more dataset collections. Requires the
...
The settings files provides dataset3d with the configuration
...
you should use the ".yaml" extension so that dataset3d will correctly
parse the YAML.
...
Now we can run dataset3d and make the dataset collection available
via HTTP.

~~~
    dataset3d start settings.yaml
~~~
...
In the shell session where dataset3d is running press "ctr-C"
to terminate the service.


dataset3d 3.0.0-alpha
