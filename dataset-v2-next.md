...
Dropping libdataset support (done in release v2.2.2) would allow
dataset/datasetd to be cross compiled for all supported platforms
using only the Go tool chain.  It would make fully supporting snap 
installs possible. Dataset's datasetd provides alternative language
support via a JSON API as a web service.
