...
- `/api/<COLLECTION_NAME>/query/<QUERY_NAME>[/<FIELD_NAMES>?PARAMS]`, query the collection and receive a list of objects in response
...
You convert the YAML list to a JSON lines documents using [datatools](https://caltechlibrary/datatools).

~~~shell
 cat <<YAML | yaml2json | json2jsonl >people_list.jsonl
- key: doe-jane
  object:
    pid: doe-jane
    family: Doe
...
In JSON lines it would look like this.

~~~json
{"key":"doe-jane","object":{"family":"Doe","lived":"Jane","orcid":"9999-9999-9999-9999","pid":"doe-jane"}}
~~~

This is the object structure used by the "dump" verb. It can record structure. You can load records using this way.

~~~shell
dataset3 load people.ds <people_list.jsonl
~~~
